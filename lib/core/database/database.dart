...
/// 幂等地删除列：列不存在则跳过。
///
/// 用于「代码回退后清理高版本残留」场景（见 [AppDatabase.migration] 的
/// 降级处理）。先查 `PRAGMA table_info` 再执行 `ALTER TABLE DROP COLUMN`，
/// 避免对不存在的列执行 DROP 抛错。
...
Future<void> downgradeCleanup(Migrator m) async {
  ...
  // DROP TABLE IF EXISTS 保证）。
  await db.customStatement('DROP TABLE IF EXISTS ai_providers');
  // v12 -> v13 新增到 settings 的 AI 供应商配置列。
...
class AppDatabase extends _$AppDatabase {
  ...
  MigrationStrategy get migration => MigrationStrategy(
    ...
    onUpgrade: (migrator, from, to) async {
      // 代码回退（如移除 AI 功能后 schemaVersion 从 14 降回 12）：
      // drift 的 step-by-step 迁移只支持升级，检测到降级时先清理
      // 高版本残留结构，让 drift 把 user_version 写回当前版本。
      if (from > to) {
        await downgradeCleanup(migrator);
        return;
      }
      await migration_steps.stepByStep(
